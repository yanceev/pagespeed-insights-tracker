...
import { displayUrl } from "@/lib/normalize-url";
...
const SITE_PREFERENCE_KEY = "psi-tracker-site";
...
export default function PSITracker() {
  ...
  useEffect(() => {
    ...
    if (saved === "wizyvision" || saved === "wizyemm" || saved === "wizy") {
      setSelectedSite(saved);
    }
  }, []);

  const loadMetrics = useCallback(async (siteId: SiteId) => {
    setIsLoading(true);
    setLoadError(null);
    try {
      const res = await fetch(`/api/metrics?siteId=${siteId}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to load metrics");
      setMetrics(data.metrics);
    } catch (e) {
      setLoadError(e instanceof Error ? e.message : "Failed to load metrics");
      setMetrics([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    loadMetrics(selectedSite);
  }, [selectedSite, loadMetrics]);
  ...
  const handleSelectSite = (siteId: SiteId) => {
    ...
    setForm((f) => ({ ...f, url: site.defaultUrl }));
    localStorage.setItem(SITE_PREFERENCE_KEY, siteId);
  };
  ...
  const handleChangeSite = () => {
    ...
    setActiveTab("overall");
    setAnalysisResult(null);
    localStorage.removeItem(SITE_PREFERENCE_KEY);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoadError(e instanceof Error ? e.message : "Failed to save metric");
    } finally {
  ...
  const deleteMetric = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `/api/metrics?siteId=${selectedSite}&id=${encodeURIComponent(id)}`,
        { method: "DELETE" }
      );
      if (!res.ok) {
        const data = await res.json();
      ...
      setLoadError(e instanceof Error ? e.message : "Failed to delete metric");
    }
  };
  ...
  if (!selectedSite) {
    return <SitePicker onSelect={handleSelectSite} />;
  }

  const site = getSite(selectedSite);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-6 md:p-12">
      <header className="max-w-7xl mx-auto mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <button
            ...
            className="flex items-center gap-1 text-sm font-bold text-slate-500 hover:text-indigo-600 mb-3 transition-colors"
          >
          ...
          <div className="flex items-center gap-4 mt-2">
            <p className="text-slate-500 text-lg">PSI velocity dashboard</p>
      ...
      {loadError && (
        <div className="max-w-7xl mx-auto mb-6 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm font-medium">
          {loadError}
        </div>
      )}

      {isLoading ? (
        <div className="max-w-7xl mx-auto flex items-center justify-center gap-2 text-slate-500 py-24">
        ...
        <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          device: e.target.value as "mobile" | "desktop",
                        })
                      }
                    >
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    URLs are normalized (https, no www) before saving.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-indigo-600 transition-colors shadow-lg shadow-slate-200 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isSubmitting && <Loader2 className="w-4 h-4 animate-spin" />}
                  Log {form.device.charAt(0).toUpperCase() + form.device.slice(1)}{" "}
                  Entry
                </button>
              </form>
            </section>
            ...
            <section className="bg-indigo-900 text-white p-6 rounded-2xl shadow-xl">
              <h3 className="text-lg font-bold mb-4 opacity-80 uppercase text-xs tracking-widest">
                {site.name} {activeDevice} snapshot
              </h3>
              ...
              <div className="space-y-8">
                <section className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 min-h-[400px]">
                  <h2 className="text-2xl font-black mb-6">
                    Velocity Overview ({activeDevice})
                  </h2>
                  {uniqueUrls.length > 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p>No {activeDevice} data logged yet for {site.name}.</p>
                    </div>
                  )}
                </section>
              ...
              <div className="space-y-8">
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      <h2 className="text-2xl font-black mb-1">
                        Timeline for {displayUrl(activeTab)} ({activeDevice})
                      </h2>
                      <p className="text-sm text-slate-500">
                        {groupedMetrics[activeTab].length} data points captured
                      </p>
                    </div>
                    <button
                ...
                {analysisResult && (
                  <section className="bg-slate-900 text-white p-8 rounded-3xl shadow-2xl border border-indigo-500/30 relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/20 blur-[100px] rounded-full -mr-20 -mt-20 group-hover:bg-indigo-600/30 transition-all duration-500" />
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
                          <p className="text-indigo-400 text-xs font-black uppercase tracking-widest mb-4">
                              ...
                              <li key={i} className="flex gap-3 text-sm">
                                ...
                                <span className="text-slate-200">
                                  {typeof opt === "string" ? (
                                    ...
                                    <>
                                      <strong className="text-indigo-300 mr-1">
                                        {opt.area}:
                                      </strong>
                                      {opt.recommendation}
                                    </>
